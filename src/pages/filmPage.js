...
function renderSimilarFilms(films) {
  if (films.length > 0) {
    const filmsWrapper = `<div class="similiar-movies"><h2>Похожие фильмы</h2><div class = "film-list single-movie-similiars">${renderSimilarCard(
      films
    )}</div></div>`;
   ...
   <img src="${film.posterUrl}" width="240" height="460" alt="">
     <div class="caption">
        ...
        <div class="content-wrap">
              <div class="content"><b>Краткое описание: </b> ${
                film.shortDescription ? film.shortDescription : "Отсутствует"
              }</div>
               <div class="content"><b>Описание: </b> ${film.description}</div>
        </div>
  
        <div class="genres item">Жанры: ${film.genres
          ...
          .join()}</div>
        <div class="countries item">Страна: <span>${
          film.countries[0].country
        }</span></div>

        <div class="length item">Длина фильма: <span> ${
          film.filmLength
        } мин.</span></div>
        <div class="rating item">Рейтинг кинопоиск: <span>${
          film.ratingKinopoisk ? film.ratingKinopoisk : "n/a"
        }</span></div>
...
function renderSimilarCard(films) {
  return films.length !== 0
    ? String(films.map((item) => similliarCard(item))).replaceAll(",", "")
    : [];
}
